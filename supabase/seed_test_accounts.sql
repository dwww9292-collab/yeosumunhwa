...
--   admin@yeosu.kr / admin1234  -> 관리자(super_admin)
--   user1@yeosu.kr / user1234   -> 일반 사용자
...
where email in ('admin@yeosu.kr', 'user1@yeosu.kr');

-- 2) admin@yeosu.kr 을 최고관리자로 승격 (profiles 행 생성)
insert into public.profiles (id, name, email, role, is_active)
select u.id, '테스트 관리자', u.email, 'super_admin', true
from auth.users u
where u.email = 'admin@yeosu.kr'
...
where u.email in ('admin@yeosu.kr', 'user1@yeosu.kr');
