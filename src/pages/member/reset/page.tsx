...
import { resolveLoginEmail } from "@/features/auth/accountId";

/** dwww92@naver.com → dw****@naver.com */
...
export default function MemberResetRequest() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      email = await resolveLoginEmail(userId);
    } catch {
      setSubmitting(false);
        ...
        {sentTo ? (
          <div className="text-center">
            ...
            <p className="text-sm text-gray-500 mt-2">
              가입 시 등록한 이메일 {maskEmail(sentTo)} 로 비밀번호 재설정 링크를 보냈습니다.
          ...
          <>
            <div className="text-center mb-8">
              ...
              <p className="text-sm text-gray-500 mt-1">
                아이디를 입력하면 가입 시 등록한 이메일로 재설정 링크를 보내드립니다.
              </p>
            </div>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={submitting}
                className="w-full bg-[#1a4fa0] text-white rounded-lg py-2.5 text-sm font-medium hover:bg-[#163f82] disabled:opacity-50 cursor-pointer"
              >
